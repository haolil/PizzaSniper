...
public class Snipe : MonoBehaviour
{
    ...
    void ShootGun()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (rayHit.collider != null && rayHit.collider.tag == "Enemy" || rayHit.collider.tag == "Window")
            {
                this.GetComponentInParent<Snipe>().aimCheck = true;
                if (rayHit.collider.tag == "Enemy")
                {
                    Pineapple = rayHit.collider.gameObject.GetComponent<PineappleAI>();
                }
                else if (rayHit.collider.tag == "Window")
                {
                    windowBreak = rayHit.collider.gameObject.GetComponent<WindowBreak>();
                }
            }
            else
            {
